...
describe("registry", () => {
  it("has demo.ping, oauth.exchange and storage.retention, and no invented storage.scan", () => {
    expect([...createRegistry(HANDLERS).keys()].sort()).toEqual(["demo.ping", "import.suggest_mapping", "oauth.exchange", "payments.refund", "payments.sync_payouts", "payments.test_charge", "payments.webhook.paypal", "payments.webhook.stripe", "storage.retention"]);
  });
