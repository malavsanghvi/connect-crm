...
export default async function BankPage({ searchParams }: { searchParams: Promise<RawSearchParams> }) {
  ...
  const retry = hrefWith("/giving/payments/bank", sp, {});
  ...
  return (
    <>
      {header}
      <form method="get" action="/giving/payments/bank" className="mb-4 flex flex-wrap items-end gap-3">
        <div>
      ...
      <BankResultsProvider>
        ...
        <Tabs
          ...
          tabs={VIEWS.map((v) => ({
            ...
            href: hrefWith("/giving/payments/bank", { account: account.id }, { view: v }),
          }))}
        />
        ...
        {res.error ? (
          ...
          <div className="space-y-3">
            ...
            <Card padded={false}>
              <Pagination page={page} pageSize={PAGE_SIZE} total={res.count ?? null} hrefFor={(n) => hrefWith("/giving/payments/bank", sp, { page: n })} />
            </Card>
          </div>
        )}
      </BankResultsProvider>
