...
export default async function CampaignsPage() {
  ...
  return (
    <>
      {header}
      {campaigns.error ? (
        <QueryError what="campaigns" error={campaigns.error} retryHref="/giving/opportunities/campaigns" />
      ) : (
        <Card padded={false} className="mb-6">
          {pledges.error ? (
            <div className="p-4">
              <QueryError what="campaign totals" error={pledges.error} retryHref="/giving/opportunities/campaigns" />
            </div>
          ) : null}
