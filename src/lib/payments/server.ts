...
//   SUPABASE_SERVICE_ROLE_KEY   (webhook routes only: app.ingest_webhook is service_role only)
...
/** Service-role client for the webhook routes (after the signature was verified). Null when not configured. */
export function serviceClient(screen: string): AppSupabase | null {
  const env = readPublicEnv();
  const key = val("SUPABASE_SERVICE_ROLE_KEY");
  if (!env.ok || !key) return null;
  return createClient<Database, "app">(env.env.supabaseUrl, key, {
    db: { schema: "app" },
    auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
    global: { headers: { "x-client-app": "job", "x-request-id": newRequestId(), "x-client-screen": screen } },
  });
}
...
/**
 * app.ingest_webhook (0210) is executable by service_role only, so it is not in
 * the generated (authenticated) types; this is its one typed call.
 */
export async function ingestWebhook(db: AppSupabase, provider: string, eventId: string, type: string, payload: Record<string, unknown>) {
  const rpc = db.rpc as unknown as (fn: string, args: Record<string, unknown>) => Promise<{ data: unknown; error: { message: string; code?: string } | null }>;
  return rpc.call(db, "ingest_webhook", { p_provider: provider, p_event_id: eventId, p_type: type, p_center: null, p_payload: payload });
}
