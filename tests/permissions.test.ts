...
describe("visibleNav (flat module list)", () => {
  ...
  it("lists the eight Settings tabs in the prototype's order (then Modules, Team, Agreements, Member app, Limits) for a platform admin, then Platform", () => {
    ...
    expect(nav.find((m) => m.key === "platform")!.tabs.map((t) => t.label)).toEqual(["Centers", "Platform setup", "New center wizard", "Verification", "Requests", "Sandbox codes", "Onboarding", "Go-live approvals", "Support access"]);
  });
