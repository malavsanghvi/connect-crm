...
describe("visibleNav (flat module list)", () => {
  ...
  it("keeps the prototype's module order", () => {
    ...
    expect(labels).toEqual(["Home", "People", "Events", "Giving", "Bolis", "Satvik Store", "Pathshala", "Content", "Calendar", "Communications", "Accounting", "Reports", "Settings", "Platform"]);
  });
  it("shows settings to a center admin and opens on the first tab they can use", () => {
    ...
    expect(settings.tabs.map((t) => t.label)).toEqual(["Rules", "Roles & entitlements", "Onboarding fields", "Notifications", "Security", "Modules", "Member app", "Limits"]);
  ...
  it("lists the eight Settings tabs in the prototype's order (then Modules, Member app, Limits) for a platform admin, then Platform", () => {
