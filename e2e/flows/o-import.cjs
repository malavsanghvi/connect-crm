...
(async () => {
  ...
  ok(sql(`select exists (select 1 from app.readiness_checks where key = 'records_imported')`) === 't', 'the readiness check "records imported and reconciled; contact coverage above target" is registered');
