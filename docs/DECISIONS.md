...
| Money rules of the 2026-09-25 batch, as built (e-money) | #5 `center_payment_processors.donor_covers_fee_allowed` is always false (check constraint; `set_payment_processor` refuses true). #6 a refund reported by the Stripe/PayPal webhook that Community Connect did not record becomes ONE flagged `payment_refunds` row per provider reference; it changes nothing until a person with giving.manage approves first and a DIFFERENT person with giving.approve approves second (reason + fresh 2FA each); the second approval records it like any refund (refunded_cents/status; allocations untouched, as for every refund). #7 email-only PayPal: after the existing request → second approver, the treasurer records the PayPal refund (amount, date, PayPal transaction id, reason; fresh 2FA). #11 a QuickBooks CreditMemo/RefundReceipt of an approved donor match reduces the one matching historical payment of that customer (its own link, else the latest same-amount payment dated on or before it, else the only payment with enough left); never posted back; the two-person trigger lets exactly this through (historical payment, background service, flag set by the bring-in). #24 opening balances: one historical `is_opening_balance` payment per pledge for what the file says was paid beyond the imported payment history (run after the payments import); written-off pledges import written off, closed, with `written_off_by_name` and `write_off_reason`; an import never writes off a pledge that exists open. Write-off → QuickBooks: completing a two-person write-off queues one `pledge_writeoff` posting: CreditMemo applied to the source QuickBooks invoice, or (accrual) a JournalEntry Pledge write-offs → Pledges receivable, else `skipped` with the reason; new mapping purpose `pledge_writeoffs`. #10 the live test post is explained before and after (app.qbo_test_post_explained). Once any refund is recorded on a payment, the approvals left on it can never send or record a second one (0415, enforcing #8). #12 (delete used OAuth codes) NOT built — not authorized. | `app.payment_refunds`, `worker_flag_provider_refund`, `approve_flagged_refund`, `record_manual_paypal_refund`, `qbo_bring_in_refund`, `enqueue_pledge_writeoff_posting`, `import_pledge_opening_balances` (0410–0415; owner, 2026-09-25) |
| Owner decisions (2026-09-25, second batch) | Data stays in one shared database with every record tagged to its organization and isolated by the database (no schema- or database-per-organization split). **JSH's current organization is a sandbox**; Community Connect's super admin can create further sandboxes directly. (12) Used OAuth authorization codes are deleted from the vault once exchanged. The owner counts as staff for the 2FA rules. The owner passes role-based checks (Executive Committee, zone lead, …) except resetting another admin's 2FA. QuickBooks posts on cash basis only until an accrual customer needs it; the basis (cash or accrual) is chosen at first QuickBooks setup. Year-end statements leave out opening-balance lines. JSH sandbox content: the live stream, the website's calendars and events, and general Jain donation opportunities. | Owner, 2026-09-25 |
| Money items of the second batch, as built (f-money) | #12 after every `oauth.exchange` (Stripe, PayPal, Intuit) the background service removes the used authorization code (`oauth.code` or the job's `code_secret`) through `app.worker_remove_oauth_code` (connect_worker only; only a secret named like an authorization code), audited "used authorization code removed after exchange"; a failed exchange removes it too unless a retry of the same job could still use it (temporary failure with attempts left). QuickBooks basis: `app.set_qbo_basis` — the first choice after connecting (accounting.manage, i.e. the treasurer or the owner, + reason), required before any mapping or approval; a later change needs accounting.manage + reason + fresh 2FA and withdraws the mapping/test-post approvals; stored in `integration_connections.settings.basis` (+ chosen/changed by/at). Posting stays cash-only: on accrual `qbo_post_ready`, readiness check 7, the setup screen and the posting queue say accrual posting isn't available yet and postings wait queued. Connections that already had a mapping without a basis were set to cash (what the poster already assumed). Year-end statements: `app.year_end_statement(household, year)` (household adults, giving staff) lists the year's gifts without `is_opening_balance` lines and says what it left out; an opening-balance line can never get `receipt_sent_at`; the giving history labels it "Opening balance". | 0520–0522 (owner, 2026-09-25) |
