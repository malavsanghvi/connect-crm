...
-- With a stand-in for app.enqueue_message that records what was sent (only in this transaction; the rollback restores o-messaging's).
begin;
create table pg_temp.sent (vars jsonb, tmpl text, purpose text, "to" text);
grant all on pg_temp.sent to authenticated;
create or replace function app.enqueue_message(p_center uuid, p_channel text, p_to text, p_template_key text, p_vars jsonb, p_purpose text)
